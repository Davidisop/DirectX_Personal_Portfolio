...
namespace David_DX
{
		...
		UINT iVertexSize)
	{
		...
		pContext->VSSetConstantBuffers(1, 1, Lighting_ConstantBuffer.GetAddressOf());
		pContext->PSSetConstantBuffers(1, 1, Lighting_ConstantBuffer.GetAddressOf());
	...
	bool  David_Obj_Compo_Func::Render(
		ID3D11DeviceContext* pContext,
		UINT iVertexSize, UINT Count)
	{
		PreRender(pContext, iVertexSize);
		PostRender(pContext, iVertexSize, Count);
		return true;
	}
