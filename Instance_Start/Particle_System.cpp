...
Particle::Particle()
{
	...
	D3DXVECTOR3 david_random_vector(cos(rand()), sin(rand()), (cos(rand()) + sin(rand()))*0.5);
	D3DXVec3Normalize(&david_random_vector, &david_random_vector);
	//노멀라이즈가 핵심이다.
	m_vAdd = david_random_vector*30;// rand()의 반환값은 int 랜덤이다.
