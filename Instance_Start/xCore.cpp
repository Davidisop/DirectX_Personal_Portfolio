...
{	
	...
	MainCamera->Set_View_Matrix(D3DXVECTOR3(0, 0, -1.015), D3DXVECTOR3(0.0f, 1.0f, 0.0f), D3DXVECTOR3(0, 0, 0));
	MainCamera->m_matView_here = MainCamera->m_matView;

	float fAspectRatio = g_rtClient.right / (FLOAT)g_rtClient.bottom;
	MainCamera->Set_Projection_Matrix( 0.25, fAspectRatio, 1.0f, 5000.0f);




	SkyBox.Create(m_pd3dDevice, L"../../data/shader/sky.hlsl", L"../../data/eye.bmp" );
...
{
	float color[4] = { 0.13f,0.13f,0.13f,1 };
...
bool	xCore::GameRender()
{
	GamePreRender();
	{
		PreRender();	//비어 있음. 사용해도 될듯.
		Render();//Sample
		PostRender(); //비어 있음. 사용해도 될 듯.
	}
	GamePostRender();
	return true;
}
